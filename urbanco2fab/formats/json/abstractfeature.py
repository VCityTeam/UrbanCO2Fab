...
class AbstractFeature(ABC):
  ...
  def validate(self):
    if(self.identifier == None):
      raise AbstractFeatureError("Feature identifier cannot be empty")
    if(self.name == None):
      raise AbstractFeatureError("Feature name cannot be empty")
    if(parse(self.existencestarttime) > parse(self.existenceendtime)):
      raise AbstractFeatureError("Existence start time greater than the end time")
    if(parse(self.storetransactionstarttime) > parse(self.storetransactionendtime)):
      raise AbstractFeatureError("Store transaction start time greater than the end time")
